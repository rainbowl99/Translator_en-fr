...
class InputEmbeddings(nn.Module):
    """ Projection between a word and a 512 dimensions vector"""
...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model: int, seq_len: int, dropout: float) -> None:
        ...
        # Creatge avecotr of shape (seq_len, 1)
...
class Layrnormalization(nn.Module):
...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def attention(query, key, value, mask, dropout: nn.Dropout):
        ...
        attention_scores = attention_scores.softmax(dim=-1) # (bathc, h, seq_len, seq_len)
    ...
    def forward(self, q, k, v, mask):
        ...
        x = x.transpose(1, 2).contiguous().view(x.shape[0], -1, self.h * self.d_k) # make the memory coniguous
